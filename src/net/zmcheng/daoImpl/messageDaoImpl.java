...
public class messageDaoImpl implements messageDao {
	...
	public List<Messages>getAllMessage(final int start,final int  length) throws Exception{
		...
		for(Message temp:result){
			Messages tempMess = new Messages(temp.getId(),temp.getSender(),temp.getContent(),temp.getTime(),temp.getReplyId());
			...
			for(int i=0;i<result2.size();i++){
				Messages tempMess2 = new Messages(result2.get(i).getId(),result2.get(i).getSender(),result2.get(i).getContent(),result2.get(i).getTime(),result2.get(i).getReplyId());
				list2.add(tempMess2);
			}
